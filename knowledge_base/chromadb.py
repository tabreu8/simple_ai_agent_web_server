...
class ChromaDBManager:
    ...
    def __init__(self):
        ...
        logger.info(
            f"ChromaDB initialized with collection '{self.collection_name}' at '{self.db_path}'"
        )
...
async def search_knowledge_base(query: str, max_results: int = 5) -> str:
    ...
    try:
        ...
        formatted_results.append(
            f"Found {len(results['documents'])} relevant document(s):\n"
        )
        ...
        for i, (doc, metadata, distance) in enumerate(
            ...
            formatted_results.append(
                f"--- Result {i+1} (Relevance Score: {1-distance:.3f}) ---"
            )
