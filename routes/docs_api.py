    ...
    try:
        ...
        if documents and documents.strip():
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid JSON in documents field: {str(e)}"
                )
        ...
        if has_json_docs:
            for doc_data in parsed_documents:
                ...
                metadata.update(
                    {"source_type": "json_input", "created_at": str(datetime.now())}
                )
            ...
            for file in files:
                ...
                if not document_parser.is_supported_file(file.filename):
                    ...
                    raise HTTPException(
                        status_code=400,
                        detail=(
                            f"File '{file.filename}' type not supported. "
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to insert documents: {str(e)}"
        )
...
async def search_documents(
    ...
    metadata_filter: Optional[str] = Query(
        None, description="JSON string for metadata filtering"
    ),
):
    """
    ...
    try:
        ...
        if metadata_filter:
            ...
            try:
                ...
                raise HTTPException(
                    status_code=400, detail="Invalid JSON in metadata_filter parameter"
                )

        # Perform search
        results = chromadb_manager.query_documents(
            query_text=query, n_results=n_results, where=where_filter
        )
...
async def update_document(doc_id: str, request: DocumentUpdateRequest):
    ...
    try:
        ...
        success = chromadb_manager.update_document(
            doc_id=doc_id, document=request.document, metadata=request.metadata
        )
...
async def get_document(doc_id: str):
    ...
    try:
        ...
        return DocumentResponse(
            id=doc["id"], document=doc["document"], metadata=doc["metadata"]
        )
