...
class TestDocumentInsert:
    ...
    def test_insert_file_upload_text(self, client: TestClient, sample_text_file):
        ...
        with open(sample_text_file, "rb") as f:
            response = client.post(
                "/docs/insert", files={"files": ("test.txt", f, "text/plain")}
            )
        ...
        assert (
            data["processing_summary"]["processed_files"][0]["filename"] == "test.txt"
        )

    def test_insert_file_upload_markdown(
        self, client: TestClient, sample_markdown_file
    ):
        """Test file upload with markdown file."""
        with open(sample_markdown_file, "rb") as f:
            response = client.post(
                "/docs/insert", files={"files": ("test.md", f, "text/markdown")}
            )
    ...
    def test_insert_file_upload_pdf(self, client: TestClient):
        ...
        assert len(data["processing_summary"]["processed_files"]) == 1
        assert (
            data["processing_summary"]["processed_files"][0]["filename"]
    ...
    def test_insert_combined_json_and_files(self, client: TestClient, sample_text_file):
        ...
        json_docs = [
            {"content": "JSON document content", "metadata": {"source": "json"}}
        ]
    ...
    def test_insert_no_content_error(self, client: TestClient):
        ...
        assert (
            "At least one of 'documents' (JSON) or 'files' must be provided"
    ...
    def test_insert_unsupported_file_type(self, client: TestClient):
        ...
        response = client.post(
            "/docs/insert",
            files={
                "files": ("test.exe", fake_file_content, "application/octet-stream")
            },
        )
    ...
    def test_deprecated_upload_endpoint(self, client: TestClient, sample_text_file):
        ...
        with open(sample_text_file, "rb") as f:
            response = client.post(
                "/docs/upload", files={"file": ("test.txt", f, "text/plain")}
            )
...
class TestDocumentSearch:
    ...
    def test_search_with_specific_query(
        self, client: TestClient, inserted_document_ids
    ):
        ...
        found_fastapi = any(
            "FastAPI" in str(result.get("document", "")) for result in data["results"]
        )
        assert (
            found_fastapi or len(data["results"]) > 0
        )  # Either found FastAPI or got other results

    def test_search_with_n_results_parameter(
        self, client: TestClient, inserted_document_ids
    ):
    ...
    def test_search_with_metadata_filter(
        ...
        response = client.get(
            f"/docs/search?query=test&metadata_filter={metadata_filter}"
        )
...
class TestDocumentCRUD:
    ...
    def test_update_document_metadata_only(
        self, client: TestClient, inserted_document_ids
    ):
    ...
    def test_update_document_no_data(self, client: TestClient, inserted_document_ids):
        ...
        assert response.status_code == 400
        assert (
            "At least one of 'document' or 'metadata' must be provided"
            in response.json()["detail"]
        )
...
class TestIntegrationFlows:
    ...
    def test_complete_document_lifecycle(self, client: TestClient):
        ...
        insert_response = client.post(
            "/docs/insert", data={"documents": documents_json}
        )
    ...
    def test_metadata_filtering_flow(self, client: TestClient):
        ...
        search_response = client.get(
            f"/docs/search?query=tutorial&metadata_filter={category_filter}"
        )
        assert search_response.status_code == 200
        ...
        python_search = client.get(
            f"/docs/search?query=programming&metadata_filter={language_filter}"
        )
