"""
Integration tests for agent with knowledge base functionality.
...
class TestAgentKnowledgeBaseIntegration:
    ...
    def test_agent_references_specific_knowledge(
        self, client: TestClient, openai_api_key
    ):
    ...
    def test_agent_searches_internal_company_data(
        self, client: TestClient, openai_api_key
    ):
        ...
        assert (
            "189,750" in response_text
            or "$189,750" in response_text
            or "189750" in response_text
        )

    def test_agent_handles_no_relevant_knowledge(
        self, client: TestClient, openai_api_key
    ):
        ...
        documents_data = [
            {
                ...
                "metadata": {"filename": "cookie_recipe.txt", "category": "cooking"},
            }
        ]
        ...
        found_indicator = any(
            indicator in response_text for indicator in no_info_indicators
        )
    ...
    def test_agent_prioritizes_knowledge_over_general_info(
        self, client: TestClient, openai_api_key
    ):
    ...
    def test_agent_maintains_knowledge_context_across_session(
        self, client: TestClient, openai_api_key
    ):
    ...
    def test_agent_combines_multiple_knowledge_sources(
        self, client: TestClient, openai_api_key
    ):
        ...
        # Hardware specs
        assert "48" in response_text and (
            "cpu" in response_text or "cores" in response_text
        )
